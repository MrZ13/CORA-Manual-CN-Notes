  ...
  - [2.1 Set Operations](#21-set-operations)
    - [2.1.1 Basic Set Operations](#211-basic-set-operations)
      ...
      - [2.1.1.2 plus](#2112-plus)
        - [1.定义](#1定义-1)
        - [2.示例](#2示例-1)
      - [2.1.1.3 cartProd(Cartesian Product 笛卡尔积)](#2113-cartprodcartesian-product-笛卡尔积)
        - [1.定义](#1定义-2)
        - [2.示例](#2示例-2)
      - [2.1.1.4 convHull(convex hull 凸包)](#2114-convhullconvex-hull-凸包)
        - [1.定义](#1定义-3)
        - [2.示例](#2示例-3)
      ...
      - [2.1.1.7 or](#2117-or)
        - [1.定义](#1定义-6)
        - [2.示例](#2示例-6)
    - [2.1.2 Predicates(谓词运算)](#212-predicates谓词运算)
      - [2.1.2.1 in](#2121-in)
        - [1.定义](#1定义-7)
        - [2.示例](#2示例-7)
    ...
    - [2.1.3 Set Properties](#213-set-properties)
      ...
      - [2.1.3.4 vertices](#2134-vertices)
        - [1.定义](#1定义-12)
        - [2.示例](#2示例-12)
      - [2.1.3.5 volume(体积)](#2135-volume体积)
        - [1.示例](#1示例-1)
  ...
  - [2.2 Set Representations](#22-set-representations)
    - [2.2.1 Basic Set Representations](#221-basic-set-representations)
      ...
      - [2.2.1.4 MPT Polytopes（MPT多面体）](#2214-mpt-polytopesmpt多面体)
        - [1.定义](#1定义-19)
          - [2.2.1.4.1 H-repre](#22141-h-repre)
          - [2.2.1.4.2 V-repre](#22142-v-repre)
          - [使用顶点表示的凸包(convex hull)](#使用顶点表示的凸包convex-hull)
        - [2.CORA中polytope的定义方式](#2cora中polytope的定义方式)
        - [3.示例](#3示例-1)
      - [2.2.1.5 Polynomial Zonotopes（多项的zonotope)](#2215-polynomial-zonotopes多项的zonotope)
        - [1.R<sup>n</sup>空间上多项式Zonotope的定义](#1rsupnsup空间上多项式zonotope的定义)
        - [2.CORA中多项式Zonotope的表达方式](#2cora中多项式zonotope的表达方式)
        - [3.示例](#3示例-2)
      - [2.2.1.6 Capsules（胶囊）](#2216-capsules胶囊)
        - [1.Capsules在R<sup>n</sup>空间上的定义](#1capsules在rsupnsup空间上的定义)
        - [2.CORA中Capsules的表示](#2cora中capsules的表示)
        - [3.示例](#3示例-3)
      - [2.2.1.7 Zonotope Bundles (zonotope束)](#2217-zonotope-bundles-zonotope束)
        - [1.Zonotope Bundles在n维空间上的定义](#1zonotope-bundles在n维空间上的定义)
        - [2.CORA中ZB的示例](#2cora中zb的示例)
        - [3.示例](#3示例-4)
      - [2.2.1.8 Constrained Zonotopes](#2218-constrained-zonotopes)
        - [1.Constrained Zonotopes的定义](#1constrained-zonotopes的定义)
        - [2.CORA中Constrained Zonotopes的表示](#2cora中constrained-zonotopes的表示)
        - [3.示例](#3示例-5)
      - [2.2.1.9 Probabilistic Zonotopes](#2219-probabilistic-zonotopes)
        - [1.定义](#1定义-20)
        - [2.CORA中Probabilistic Zonotopes的表示](#2cora中probabilistic-zonotopes的表示)
        - [3.示例](#3示例-6)
    - [2.2.2 Auxiliary(辅助的) Set Representations](#222-auxiliary辅助的-set-representations)
    ...
    - [2.2.3 Set Representations for Range Bounding](#223-set-representations-for-range-bounding)
      ...
      - [2.2.3.1 Taylor Models](#2231-taylor-models)
        - [1.定义](#1定义-25)
        - [2.示例](#2示例-23)
      - [2.2.3.2 Affine（仿射）](#2232-affine仿射)
        - [1.定义](#1定义-26)
        - [2.示例](#2示例-24)
...
在CORA中，使用affine类来实现仿射。不过，由于仿射相当于一阶的泰勒展开，因此仿射类继承了泰勒模型类，且自身并不实现任何额外的功能。因此可以理解为，仿射类就是一种为了更简单地使用而构造的taylm类的包装类。



CORA中affine的使用方式如下(第二行中的参数和泰勒模型的一样)
...
#### 2.2.3.3 Zoo
> 在需要使用范围边界时，使用多种算法并行计算，取其结果的交集，其效果往往比只是用一种方法更好。因此，在CORA中可以使用Zoo方法，来方便地指定选择哪几种方法进行混合计算
...
```
> 除了methods以外，其余的参数和taylm方法是一样的
>
> methods参数代表选择的算法，备选项如下：
>
> - interval
> - affine(int)
> - affine(bnb)
